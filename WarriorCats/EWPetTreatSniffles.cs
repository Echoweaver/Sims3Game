...
{
	public class EWPetTreatSniffles : EWAbstractPetTreatPlantable
	{
		public class Definition : InteractionDefinition<Sim, GameObject, EWPetTreatSniffles>
		{
			public override string GetInteractionName(Sim actor, GameObject target, InteractionObjectPair iop)
			{
				return "EWPetSniffles" + Localization.Ellipsis;
			}
				...
				ref GreyedOutTooltipCallback greyedOutTooltipCallback)
			{
				...
				{
					greyedOutTooltipCallback = CreateTooltipCallback("Localize - No sims with sniffles");
					return false;
				}
				return true;
			}
		...
		public static List<Sim> GetTreatableSims(Sim actor, Lot lot)
		{
			...
			if (!lot.IsWorldLot)
			{
				...
				{
					if (s != actor && s.BuffManager.HasElement(LoadThis.buffNameSniffles))
					{
						Lazy.Add(ref list, s);
					}
				}
			...
			{
				if (sim != actor && sim.BuffManager.HasElement(LoadThis.buffNameSniffles)
		...
		{
			badBuff = simToPresentTo.BuffManager.GetElement(LoadThis.buffNameSniffles);
			if (badBuff == null)
			{
				return false;
			}
